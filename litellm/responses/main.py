...
def _should_fake_responses_stream(
    ...
    """Allow an explicit deployment capability to override provider defaults."""
    provider_default = responses_api_provider_config.should_fake_stream(
        model=model,
        stream=stream,
        custom_llm_provider=custom_llm_provider,
    )
    if stream is not True or not isinstance(model_info, dict):
        return provider_default

    supports_native_streaming = model_info.get("supports_native_streaming")
    if supports_native_streaming is True:
        return False
    ...
    **kwargs,
    ...
    try:
        ...
        response = base_llm_http_handler.response_api_handler(
            ...
            fake_stream=_should_fake_responses_stream(
                responses_api_provider_config=responses_api_provider_config,
                model=model,
                stream=stream,
                custom_llm_provider=custom_llm_provider,
