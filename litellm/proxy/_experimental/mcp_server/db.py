...
async def purge_user_oauth_credentials_for_server(
    ...
    table = prisma_client.db.litellm_mcpusercredentials
    rows = await table.find_many(where={"server_id": server_id})
    if not rows:
        return 0
    await table.delete_many(where={"server_id": server_id})
    ...
    for row in rows:
        try:
            ...
            verbose_proxy_logger.warning(
                "Failed to drop cached MCP OAuth token for user=%s server=%s: %s",
                row.user_id,
                server_id,
                exc,
            )
