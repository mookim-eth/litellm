...
client = openai.OpenAI(
	api_key="${v||"YOUR_LITELLM_API_KEY"}",
	base_url="${I}"
)`;switch(_){case r.CHAT:{let e=Object.keys(C).length>0,a="";if(e){let e=JSON.stringify({metadata:C},null,2).split("\n").map(e=>" ".repeat(4)+e).join("\n").trim();a=`,
    extra_body=${e}`}let i=O.length>0?O:[{role:"user",content:y}];t=`
import base64
...
if image_data:
	...
	print(response)
`;break;case r.IMAGE_EDITS:t="azure"===A?`
import base64
import os
import time
import json
